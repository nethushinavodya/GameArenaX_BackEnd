...
public interface StreamerRepo extends JpaRepository<Streamer, String> {
    ...
    @Modifying
    @Query(value = "UPDATE Streamer s SET s.country = ?1, s.bio = ?2,s.display_name = ?3, s.platform = ?4, s.stream_url = ?5 WHERE s.email = ?6", nativeQuery = true)
    void updateStreamer(String country, String bio, String displayName, String platform, String streamUrl, String email);
